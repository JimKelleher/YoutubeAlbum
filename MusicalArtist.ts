...
class musical_artist {
    ...
    convert_common_abbreviations(look_in: string): string {
    ...
    does_video_artist_require_quotes(artist_match_key: string): boolean {
    ...
    fix_artist_misspellings(look_in: string): string {
    ...
    get_artist_match_key(artist_cored: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let artist_match_key: string = artist_cored;
    ...
    get_artist_name_core(artist: string): string {
    ...
    get_reduced_character_set(artist_or_album_or_video: string): string {
        ...
        let artist_or_album_or_video_edited: string = artist_or_album_or_video;
    ...
    get_title_core(title_match_key: string, artist_match_key: string): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let title_match_key_edited: string = title_match_key;
    ...
    get_title_match_key(album_or_video: string, artist_match_key: string): string {

        // This function harmonizes the titles of a Wikipedia album and a Youtube video so they can be compared to one another.

        // Since we will be modifying the argument, let's make a copy and work with that:
        let title_match_key_edited: string = album_or_video;
    ...
    is_artist_a_common_word(wikipedia_artist: string): boolean {
        ...
        let common_word_artist: string[] = [
            ...
            "Journey",
            "Kansas",
            "Manfred Mann's Earth Band",    // this is not a common word problem but, when creating a "core" name, we don't want to strip the trailer ("band")
            "Renaissance",
            "Sweet",
            "The Association",
            "The Band",                     // this is the artist name core rule exception (to avoid absurdity) for both leaders ("the") and trailers
            ...
            "The Guess Who",
            "The Motels",
            "The The",                      // this is the artist name core rule exception (to avoid absurdity) for leaders
            "The Tubes",
            "The Who",
            "The Zombies",                  // "They Might Be Zombies" was a false positive
            "Toto",                         // is a common first name in Italian
            "Yes"
        ...
        for (let i: number = 0; i < common_word_artist.length; i++) {
    ...
    is_artist_a_place_name(artist_match_key: string): boolean {
        ...
        */
        ...
        for (let i: number = 0; i < place_name.length; i++) {
    ...
    regularize_artists(artist_or_title_match_key, artist_match_key): string {

        // Since we will be modifying the argument, let's make a copy and work with that:
        let match_key: string = artist_or_title_match_key;
